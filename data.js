...
async function loadData() {
  if (isCurrentlyLoading) {
    console.log('[DADOS] Carregamento já em andamento.');
    return;
  }
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    try {
      ...
      console.log('[DADOS] Navegando até a pasta de destino...');
      await client.cd("portal/fiscalizacao/seguranca-e-saude-notrabalho/caepi");
      console.log(`[DADOS] Pasta encontrada. Agora no diretório: ${await client.pwd()}`);
      
      console.log('[DADOS] Passo 1/4: Baixando o arquivo .zip...');
      const writable = new PassThrough();
      await client.downloadTo(writable, "tgg_export_caepi.zip");
      ...
      console.log('[DADOS] Passo 3/4: Lendo e processando o arquivo .dbf... (Pode demorar)');
...
function getCAInfo(caNumber) {
  ...
  if (caInfo) {
    return {
      'Nº do CA': caInfo.NR_CA, 'Data de Validade': caInfo.DT_VALIDADE,
      'Situação': caInfo.DS_SITUACAO, 'Equipamento': caInfo.DS_EQUIPAMENTO,
